    ...
    const obj = {
      ...
      code: lineCode,
        ...
        init: function(){
          ...
          this.line.classList = "accordion-item"
          ...
          const collapseCtrl = this.line.appendChild(document.createElement("div"))
          collapseCtrl.id = collapseCtrlID
          collapseCtrl.classList = "accordion-collapse"
          collapseCtrl.classList.add(collapseClass)
          if (show){
          ...
          const dbody = collapseCtrl.appendChild(document.createElement("div"))
          dbody.classList = "accordion-body"
          let dbodyA = dbody.appendChild(document.createElement("div"))
          dbodyA.classList = "row"    

          ctrl = dbodyA.appendChild(document.createElement("div"))
          ctrl.classList = "col"

          this.line_header = createInput("line_header" + this.id, "", false, "checkbox", "form-check-input")                    
          ctrl.appendChild(createDivCtrl("form-check", this.line_header, "Essa linha é cabeçalho"))
          ...
          let divBtns = dbodyA.appendChild(document.createElement("div"))
          divBtns.classList = "col-auto"
          const btnAddCol = divBtns.appendChild(document.createElement("btn"))
          btnAddCol.classList = "btn btn-info me-1"
          btnAddCol.innerHTML = "Add Coluna"
          btnAddCol.type = "button"

          ctrl = divBtns.appendChild(document.createElement("btn"))
          ...
          dbodyA = dbody.appendChild(document.createElement("div"))
          dbodyA.classList = "row"    
          ...
          let div = dbodyA.appendChild(document.createElement("div"))
          div.classList = "table-container text-center"
...
function getCols(obj){
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    let colValues = []
     for (let x = 0; x < 7; x++) {
       colValues.push(r[x].textContent)
     }
     for (let x = 7; x < 11; x++) {
         colValues.push("")
     }
    colValues.push(r[7].textContent)
    cols.push(colValues)    
  } 
  return cols
}
function getLine(obj){
  return {values: [obj.code, "", obj.line_level.value, "", "", obj.line_parente.value, "", "", "", "", obj.table_sulfix.value, obj.line_descr.value], cols: getCols(obj)};  
}
function getLines(){
  let aa = accordionFile.querySelectorAll('.accordion-button.line-header');
  let lines = []
    for (let i = 0; i < aa.length; i++) {
        lines.push(getLine(aa[i].obj))
    }   
    return lines
}
...
function saveToFile(){
  ...
  lines.forEach(line => {
    ...
    line.cols.forEach(line => {
      csvContent += "C" + joinList(line) + "\n"
    })
  });
